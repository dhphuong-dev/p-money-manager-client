...
const model = ref<IRegisterBody>({ fullName: '', email: '', password: '' });
...
const rules = {
  ...
  confirmPassword: {
    required: true,
    validator: (rule: FormItemRule, confirmPassword: string) =>
      confirmPasswordValidator(rule, confirmPassword, model.value.password),
    trigger: 'blur'
  }
};
